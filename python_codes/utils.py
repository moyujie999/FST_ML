...
ra, taxa, design = load_RF_task(time = 11,domain = 'prokaryotes', depth = [0,1,3])
...
def load_dominant_ASVs(domain = 'prokaryotes', depth = 0, random = False):
    '''
    # 0:Manure, 1:synthetic fertilizer, 2: legume
    # 0: reduced till, 1:till
    # 0: cover crop, no cover crop
    ...
    abundant_ASVs = (ra.mean(axis=0).ge(0.0001)) | (ra.mean(axis=0).ge(0.0001))
    abundant_ASVs_list = abundant_ASVs[abundant_ASVs == True].index.tolist()
    abundantASVs = ra[abundant_ASVs_list]
    taxa = taxa.loc[abundant_ASVs_list]


    return design,abundant_asvs, taxa
